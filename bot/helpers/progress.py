...
__all__ = ['Progress', 'humanbytes', 'TimeFormatter']
...
class Progress:
    ...
    async def progress_for_pyrogram(self, current: int, total: int, ud_type: str, start: time):
        ...
        if self.is_cancelled:
            ...
            if os.path.exists("/app/busy.txt"):
                os.remove("/app/busy.txt")
        ...
        if round(diff % float(5)) == 0 or current == total:
            try:
                ...
                try:
                    ...
                    logger.warning(f"{fd} Flood Sleep")
